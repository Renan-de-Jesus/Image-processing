...
public class Difference implements ImageFilter2 {
    @Override
    public ImageMatrix apply(ImageMatrix img1, ImageMatrix img2) {
        ...
        for (int y = 0; y < img1.getHeight(); y++) {
            for (int x = 0; x < img1.getWidth(); x++) {
                out.r[y][x] = clamp(Math.abs(img1.r[y][x] - img2.r[y][x]));
                out.g[y][x] = clamp(Math.abs(img1.g[y][x] - img2.g[y][x]));
                out.b[y][x] = clamp(Math.abs(img1.b[y][x] - img2.b[y][x]));
